...
function createDetailElement(serie) {
    ...
    detailDiv.classList.add('detail', 'row', 'flex-wrap', 'my-5', 'py-5');
    ...
    titleContainer.classList.add('title-container', 'd-flex', 'justify-content-between');
    ...
    genre.classList.add('genre', 'd-flex', 'justify-content-between');
    ...
    runtime.classList.add('runtime', 'd-flex', 'justify-content-between');
    ...
    createdBy.classList.add('createdBy', 'd-flex', 'justify-content-between');
    ...
    originalCountry.classList.add('originalCountry', 'd-flex', 'justify-content-between');
    ...
    releaseDate.classList.add('releaseDate', 'd-flex', 'justify-content-between');
    ...
    lastAirDate.classList.add('lastAirDate', 'd-flex', 'justify-content-between');
    ...
    networks.classList.add('networks', 'd-flex', 'justify-content-between');
    ...
    seasonsContainer.classList.add('seasons', 'd-flex', 'justify-content-between');
    ...
    episodesContainer.classList.add('episodes', 'd-flex', 'justify-content-between');
    ...
    const SeasonsContainer = document.createElement('div');
    SeasonsContainer.classList.add('seasons', 'd-flex');
    const SeasonsHeading = document.createElement('p');
    SeasonsHeading.classList.add('subject', 'my-1');
    SeasonsHeading.textContent = 'Saisons';
    SeasonsContainer.appendChild(SeasonsHeading);
    containerRight.appendChild(SeasonsContainer);

    const seasonsList = document.createElement('ul');
    seasonsList.classList.add('seasonsList');
    ...
    if (seasons && seasons.length) {
        for (let i = 0; i < seasons.length; i++) {
          ...
          console.log(currentSeason.episodes);
          if (currentSeason.episodes && currentSeason.episodes.length) {
            for (let j = 0; j < currentSeason.episodes.length; j++) {
              ...
              episodeTitle.textContent = currentSeason.episode_number + ' - ' + currentSeason.episodes[j].name + ' - ' + currentSeason.episodes[j].overview;
      ...
      containerRight.appendChild(seasonsList);
...
async function fetchDetail(serieId) {
    try {
        const response = await fetch(`https://api.themoviedb.org/3/tv/${serieId}?language=fr-FR&append_to_response=seasons`, options);
